...
class BlockLess34(nn.Module):
    def __init__(self, in_chan, out_chan, stride=2, downsample=None):
...
def layer_make(block, out_chan, repeat_num, stride):
    """
    ...
    if out_chan == 64:
        layers = [block(out_chan, out_chan, stride)]
    else:
        layers = [block(int(out_chan / 2), out_chan, stride, downsample)]
...
class ResNet(nn.Module):
    def __init__(self, layers, image_chan, num_classes):
        """
        默认图片大小为224*224
        ...
        self.layer1 = layer_make(BlockLess34, 64, layers[0], 1)
        self.layer2 = layer_make(BlockLess34, 128, layers[1], 2)
        self.layer3 = layer_make(BlockLess34, 256, layers[2], 2)
        self.layer4 = layer_make(BlockLess34, 512, layers[3], 2)
...
def ResNet18(image_chan=3, num_classes=1000):
    return ResNet([2, 2, 2, 2], image_chan, num_classes)


def ResNet34(image_chan=3, num_classes=1000):
    return ResNet([3, 4, 6, 3], image_chan, num_classes)
