...
class PathDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        for img in self.path_img_path:
            ...
            for i in range(len(lines) - 1):
                pad = []
                for k in range(tgt_len - i - 1):
                    pad += [(0, 0)]
                self.split_data += [(img, pad + lines[0:i + 1], pad[0:len(pad) - 1] + lines[0:i + 2])]
    ...
    def __getitem__(self, idx):
        ...
        output_patches = cut_patch(img, output_paths)
        # return img, torch.stack(input_patches), torch.stack(output_patches), torch.tensor(input_paths), torch.tensor(
        #     output_paths)

        return torch.stack(input_patches), torch.tensor(input_paths), torch.tensor(
            output_paths[len(output_paths) - 1]), img
